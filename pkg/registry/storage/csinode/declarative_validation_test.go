...
func testDeclarativeValidateUpdate(t *testing.T, apiVersion string) {
	testCases := map[string]struct {
		...
		"missing nodeID in update": {
			oldObj: mkValidCSINodeDriverNode(func(d *storage.CSINodeDriver) {}),
			updateObj: mkValidCSINodeDriverNode(func(d *storage.CSINodeDriver) {
				d.NodeID = ""
			}),
			expectedErrs: field.ErrorList{
				field.Required(
